...
export function classifySet(ids, cardMap) {
...
export function decompose(hand, cardMap, bondSet) {
    ...
    for (let j = i + 1; j < 8; j++) {
      ...
      for (let a = 0; a < 5; a++) {
        for (let b = a + 1; b < 5; b++) {
          ...
          const infoA = classifySet(setA, cardMap);
          if (!infoA) continue;
          const infoB = classifySet(setB, cardMap);
          if (!infoB) continue;
          out.push({
...
export function isCompleteForm(hand, cardMap, bondSet) {
  return decompose(hand, cardMap, bondSet).length > 0;
}

// ---- 텐파이/대기 판정 ----
// 손패 7장에 어떤 카드가 오면 완성형이 되는지 전 종류를 대입해 조사.
// declarableOnly=true면 최소 1역(D35)까지 요구 — evalFn을 주입받아 판정.
export function waitsFor(hand7, cardMap, bondSet, allCardIds, evalFn) {
  const waits = [];
  for (const id of allCardIds) {
    const hand8 = hand7.concat([id]);
    const decomps = decompose(hand8, cardMap, bondSet);
...
export function isFormalTenpai(hand7, cardMap, bondSet, allCardIds) {
  return waitsFor(hand7, cardMap, bondSet, allCardIds, null).length > 0;
}
