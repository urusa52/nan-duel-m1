...
const rules = { allowCrossGenreRun: false, minYakuToDeclare: 2 };
...
{
  const sets = formedSets(['mu-1','mu-2','mu-4','sf-1','ro-1','ro-2','ho-1','ho-3'], ctx);
  t('같은 장르 세트 탐지 (무협)', sets.some((x) => x.startsWith('무협')));
}
{
...
{
  const yk = reachableYaku(['mu-1','sf-1','fa-1','ro-1','ho-1','mu-2','sf-2','fa-2'], ctx);
  t('오대 장르 후보 (장르 다수)', yk.some((y) => y.name === '오대 장르'));
}
